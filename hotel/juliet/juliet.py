...
if(True):
    ...
    alpha = 25e-5 #calving rate coefficient [50m y^-1 meter^-1]

    # Uc = U0 - alpha F >> U0 = Uc(init) + alpha F(init)
    U0 = data.Uc + alpha * data.force()  #unbuttressed calving [m/yr], usually ~7-8000
    
    print(f"alpha {alpha:.2e}, U0 {U0:.1f}, F0 is {U0/alpha:3.2e} N/m")
    print(f"{iList[-1]} steps to run")
    print(f"initially H0:{data.H0:7.2f} m, L:{data.L:9.2f} m, Uc {data.Uc:.1f} m/y")
    for i in iList: 
        data.X_externalGrid = X_fjord
        ...
        if(i % 10 == 0):
            print(f"Step {i:03d} with H0:{data.H0:7.2f} m, L:{data.L:9.2f} m, Uc {data.Uc:.1f} m/y")
        if(i % 1 == 0):
            data.save(f'results/jRun_a{int(alpha*1e6):03d}_B{int(np.abs(np.max(Bview)*10)):02d}_{i:05d}.pickle')
